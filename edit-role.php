...
if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $query_role = mysqli_query($connect, "SELECT * FROM roles LEFT JOIN role_has_permissions ON roles.id = role_has_permissions.role_id WHERE roles.id = '$id'");
        $role_data = array();
...
if(isset($_POST['submit'])){
        if ($update) {
            ...
            if (count($in_permissions) > 0) {
                ...
                $update_rl_has_pms = mysqli_query($connect, "INSERT INTO role_has_permissions(permission_id,role_id) VALUE ('{$pms_id}','{$role_id}')");
                }
